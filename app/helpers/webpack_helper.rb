module WebpackHelper
  ...
  def get_src(app_name, chunk_name, kind)
    if Rails.configuration.webpack[:use_manifest]
      ...
      "#{Rails.application.secrets.assets_url}:8080/#{chunk_name}#{suffix}"
    end
  end
  ...
  def webpack_manifest_script(app_name)
    manifest = Rails.configuration.webpack[:common_manifest][app_name].to_json
    javascript_tag "window.webpackBundleManifest = #{manifest}"
  end
